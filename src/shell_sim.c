...
int main(void)
{
	...
	while (1)
	{
		...
		// Implemented by my self, helpfull to clear screen sometimes when testing.
		else if (same_string("clear", argv[0]))
		{
...
{
	int fd, size, ev;
	char buf[STAT_SIZE], type, refs, direct;

	if ((fd = fs_open(filename, MODE_RDONLY)) < 0) // edited to same as more to trigger error if file not found
	{
	...
	bcopy(&buf[3], (char *)&size, sizeof(int));
	printf("Stats\n"
		   "Filename: %s Type: %s Refs: %d Data block: %d Size: %d\n",
		   filename, (type == 1) ? "File" : "Directory", refs, direct, size);
	if ((ev = fs_close(fd)) < 0)
		print_fse(ev);
}
