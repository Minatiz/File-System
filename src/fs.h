...
/* This mode is used to mark a file descriptor table as used */
#define MODE_USED 1
...
// A directory is a special file that contains list of filenames and their corresponding inode numbers.
//  When file is made the filename and inode number are added to directory file
/* A directory entry */
struct dirent
{
	inode_t inode;
...
// We can store max 25 directory entries in a block, so each entry takes (Block_size /25)= 20 bytes
#define DIRENTS_PER_BLK (BLOCK_SIZE / sizeof(struct dirent)) // 25
